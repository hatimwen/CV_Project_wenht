...
from utils.loss import CrossEntropyLabelSmooth, CrossEntropy, KDLoss
...
# model
parser.add_argument('--arch', '-a', metavar='ARCH', default='se_resnext_3474',
                    help='model architecture: se_resnext_3474')
parser.add_argument('--T', '--temperature', default=1, type=float, metavar='T',
                    help='temperature of teacher mdoel')
parser.add_argument('--alpha', default=0.5, type=float, metavar='ALPHA',
                    help='alpha of teacher mdoel')

parser.add_argument('--kdarch', '-kd', metavar='ARCH', default='',
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        trainObj, top1, top5 = train_kd(train_loader, model, teacher_model, t_criterion, optimizer, epoch)
...
def train_kd(train_loader, model, teacher_model, t_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.mixup:
            ...
            output = model(input)
            t_output = teacher_model(input)
            loss = lam * t_criterion(output, target_a, t_output) + (1 - lam) * t_criterion(output, target_b, t_output)
        else:
            # compute output
            output = model(input)
            t_output = teacher_model(input)
            loss = t_criterion(output, target, t_output)
